...
setHistogram(){
  if(typeof this.chart!='undefined'){
    //this.chart.set("dataPointWidth",Math.ceil(this.chart.axisX[0].bounds.width/this.chart.data[0].dataPoints.length),true);
    this.chart.set("dataPointWidth",this.state.histWidth);
    this.chart.set("exportEnabled",true);
    console.log(this.chart.options.dataPointWidth)
    //this.chart.options.dataPointWidth = 130
...
handleChange(checked) {
  ...
  this.setState({histWidth: Math.ceil(this.chart.axisX[0].bounds.width/this.chart.data[0].dataPoints.length)})
  this.chart.set("dataPointWidth", this.state.histWidth)
  //console.log(checked)
...
handleDisplay=()=>{
  ...
  render(){
    ...
    const dataHistPts = this.setDatapointsHist()
   this.setHistogram(); 
  ...
  // axios.post(variables.API_URL+'visualize', { selected: this.state.selected,dpcheck: this.state.checked , grph_type: this.state.type})
  //   .then(response => this.setState({ coordinates: response.data }));
    console.log(this.state.selected)
    
    axios.post(variables.API_URL+'visualize',  { selected: this.state.selected,dpcheck: this.state.checked, grph_type: event.target.value})
    .then(response => {
      this.setState({ coordinates: response.data});
 ...
 var label;
  ...
  const options_histogram = {
    title: {
      text: "Histogram"
    },
     dataPointWidth: this.state.histWidth,
    data: [
    {
      type: "column",
      dataPoints: dataHistPts
    }
    ]
  }
